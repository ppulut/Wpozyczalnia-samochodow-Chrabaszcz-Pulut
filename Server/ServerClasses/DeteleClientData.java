        ...
        if(tokens.length == 3) {
            ...
            if (type.equalsIgnoreCase("admin")) {
                if (!validateAdminData.checkAdminExist(value1)) {
                    ...
                    delete_Admin = ("DELETE FROM admin WHERE id='" + value1 + "'");
                }
            } else if (type.equalsIgnoreCase("car")) {
                if (!validateAdminData.checkCarExist(value1)) {
                    return super.errorStr;
                } else {
                    delete_Admin = ("DELETE FROM spis_samochodow WHERE id='" + value1 + "'");
                }
            } else if (type.equalsIgnoreCase("user")) {
                if (!validateAdminData.checkUserExist(value1)) {
                    return super.errorStr;
                } else {
                    delete_Admin2 = ("DELETE FROM kary WHERE Klient_id='" + value1 + "'");
                    delete_Admin = ("DELETE FROM uzytkownicy WHERE id='" + value1 + "'");
                }
                ...
                if (!validateAdminData.checkRentExist(value1)) {
                    ...
                    delete_Admin = ("DELETE FROM wypozyczenia_samochodow_przez_klientow WHERE id='" + value1 + "'");
                ...
                if (!validateAdminData.checkDebtExist(value1)) {
                    ...
                    delete_Admin = ("DELETE FROM kary WHERE id='" + value1 + "'");
                }
            }
